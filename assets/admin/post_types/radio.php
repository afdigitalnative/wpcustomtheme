...
// Load Scripts
function load_radio_scripts() {
	if( get_post_type() == 'radio' )
	wp_register_script( 'upload', THEME_DIR . '/js/upload.js');
	wp_enqueue_script( 'upload' );
}
add_action( 'admin_enqueue_scripts', 'load_radio_scripts' );

// Radio Taxonomy
register_cuztom_taxonomy( 'Radio Series', 'radio', array('hierarchical' => true, 'rewrite' => array('slug' => 'radio')));

// Radio Post Type
register_cuztom_post_type( 'Radio', array('labels' => array('name' => 'Radio','menu_name' => 'Radio'),'has_archive' => 'radio', 'menu_position' => 6, 'supports' => array('title', 'editor', 'thumbnail', 'comments'), 'rewrite' => array('slug' => 'radio')) );

// Rewrite the Radio URL
function radio_rewrite() {
	$cpt = 'radio';
	$tax = 'radio_series';
...
add_action('init','radio_rewrite');

// Update Wordpress with the rewrite
function radio_link( $link, $post = 0 ) {
	$cpt = 'radio';
	$tax = 'radio_series';
	$parent = null;
    ...
    if ( $post->post_type == $cpt && $terms ) {
        return home_url(user_trailingslashit($cpt.'/'.$parent.'/'.$child.'/'.$post->post_name));
    } else if ( $post->post_type == $cpt && !$terms ) {
    	return home_url(user_trailingslashit($cpt.'/'.$child.'/'.$post->post_name));
    } else {
        return $link;
    }
}
add_filter('post_type_link', 'radio_link', 1, 3);

// Change 'Enter Title Here' Text
function radio_change_title( $title ){
  global $post;
  if( 'radio' == $post->post_type ):
    $title = 'Enter Radio Title';
  endif;
  return $title;
}
add_filter( 'enter_title_here', 'radio_change_title' );

// Add the Meta Box
function add_radio_meta_box() {
	add_meta_box(
		'radio_options', 		// $id
		'Radio Options', 		// $title
		'show_radio_options',  // $callback
		'Radio', 				// $page
		'normal', 				// $context
		'high' 					// $priority
	);
}
add_action('add_meta_boxes', 'add_radio_meta_box');
...
// The Callback
function show_radio_options() {

global $radio_meta_fields, $post;
...
echo '<input type="hidden" name="metabox_nonce" value="'.wp_create_nonce(basename(__FILE__)).'" />';
	...
	foreach ($radio_meta_fields as $field) {
...
// Save the Data
function save_radio_meta($post_id) {
    global $radio_meta_fields;
	// verify nonce
...
add_action('save_post', 'save_radio_meta');
