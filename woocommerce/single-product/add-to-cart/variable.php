...
<form class="variations_form cart" method="post" enctype='multipart/form-data' data-product_id="<?php echo $post->ID; ?>" data-product_variations="<?php echo esc_attr( json_encode( $available_variations ) ) ?>">
	<ul class="options group">
			<?php $loop = 0; foreach ( $attributes as $name => $options ) : $loop++; ?>
				<li class="variation col-4">
					<label for="<?php echo sanitize_title($name); ?>"><?php echo wc_attribute_label( $name ); ?></label>
					
					<span class="select"><select id="<?php echo esc_attr( sanitize_title($name) ); ?>" name="attribute_<?php echo sanitize_title($name); ?>">
	...
	<div class="single_variation_wrap">
		...
		<div class="variations_button">
			...
			<button type="submit" class="btn"><?php echo apply_filters('single_add_to_cart_text', __( 'Add to cart', 'woocommerce' ), $product->product_type); ?></button>
		</div>
	</div>
	<div>
